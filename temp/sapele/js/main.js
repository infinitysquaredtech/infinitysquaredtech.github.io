...
(function () {
  ...
  function validateFields(prefix) {
    ...
    if (!name) {
      ...
      setError(p + 'name', p + 'name-error', 'Name must be at least 2 characters.');
      isValid = false;
    } else {
    ...
    if (!email) {
      setError(p + 'email', p + 'email-error', 'Please enter your email address.');
      isValid = false;
    } else if (!isValidEmail(email)) {
      setError(p + 'email', p + 'email-error', 'Please enter a valid email address.');
      isValid = false;
    } else {
    ...
    if (!phone) {
      setError(p + 'phone', p + 'phone-error', 'Please enter your phone number.');
      isValid = false;
    } else if (!isValidPhone(phone)) {
      setError(p + 'phone', p + 'phone-error', 'Please enter a valid phone number (10+ digits).');
      isValid = false;
    } else {
  ...
  function attachLiveValidation(prefix) {
    ...
    document.getElementById(p + 'name').addEventListener('blur', function () {
      if (!this.value.trim()) { setError(p + 'name', p + 'name-error', 'Please enter your full name.'); }
      else if (this.value.trim().length < 2) { setError(p + 'name', p + 'name-error', 'Name must be at least 2 characters.'); }
      else { clearError(p + 'name', p + 'name-error'); }
    });
    document.getElementById(p + 'email').addEventListener('blur', function () {
      if (!this.value.trim()) { setError(p + 'email', p + 'email-error', 'Please enter your email address.'); }
      else if (!isValidEmail(this.value)) { setError(p + 'email', p + 'email-error', 'Please enter a valid email address.'); }
      else { clearError(p + 'email', p + 'email-error'); }
    });
    document.getElementById(p + 'phone').addEventListener('blur', function () {
      if (!this.value.trim()) { setError(p + 'phone', p + 'phone-error', 'Please enter your phone number.'); }
      else if (!isValidPhone(this.value)) { setError(p + 'phone', p + 'phone-error', 'Please enter a valid phone number (10+ digits).'); }
      else { clearError(p + 'phone', p + 'phone-error'); }
    });
  }
  ...
  function submitEnquiry(data, onSuccess) {
    ...
    setTimeout(function () {
      console.info('[Sapele Greens] Enquiry submitted:', data);
      onSuccess();
    }, 1200);
  }
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      submitEnquiry(data, function () {
        contactForm.innerHTML = [
          ...
          '  <h3 style="color:var(--green);margin-bottom:0.75rem;font-size:1.4rem;">Enquiry Sent!</h3>',
          '  <p style="color:var(--text-mid);font-size:0.95rem;line-height:1.7;">',
          '    Thank you for your interest in Sapele Greens.<br />',
  ...
  function triggerBrochureDownload() {
    ...
    link.href = 'brochure/sapele-greens-brochure.pdf';
    link.download = 'Sapele-Greens-Brochure.pdf';
  ...
  document.addEventListener('keydown', function (e) {
    if (e.key === 'Escape' && brochureModal && brochureModal.classList.contains('open')) {
      closeBrochureModal();
    }
  });
    ...
    brochureForm.addEventListener('submit', function (e) {
      ...
      submitEnquiry(data, function () {
        brochureForm.innerHTML = [
          ...
          '  <h3 style="color:var(--green);margin-bottom:0.5rem;font-size:1.2rem;">Download Starting&hellip;</h3>',
          '  <p style="color:var(--text-mid);font-size:0.88rem;line-height:1.7;">',
          '    Thank you! Your Sapele Greens brochure is downloading.',
          '  </p>',
          '</div>',
        ].join('');
